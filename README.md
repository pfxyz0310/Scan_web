# 📚 電子書自動截圖轉 PDF 工具 (Chrome 接管版)

這是一個 Python 自動化工具，專門用於將網頁版電子書備份為 PDF 檔案。

本程式採用 **「瀏覽器接管模式 (Remote Debugging)」** 運作。與傳統爬蟲不同，您需要先開啟一個特定的 Chrome 視窗並手動登入網站，程式會直接連接該視窗進行操作。這種方式完美解決了圖形驗證碼、帳號登入以及動態載入頁面的問題。
...
- **免帳密輸入**：直接使用您已登入的瀏覽器視窗，無需在程式碼中輸入密碼，安全性更高。
...
### 步驟 1：啟動專用瀏覽器
雙擊資料夾中的 **`start_chrome.bat`**。
- 這會跳出一個乾淨的 Chrome 視窗。
- **首次使用**：請在此視窗前往電子書網站並登入您的帳號。

### 步驟 2：準備電子書
1. 在該 Chrome 視窗中，點擊打開您要下載的電子書。
2. 等待書本載入完成，並確認已經翻到 **第 1 頁**。
3. **【非常重要】請用滑鼠點擊一下電子書內容的正中央。**
   > **為什麼要這樣做？** 這是為了讓瀏覽器取得「焦點 (Focus)」，確保等一下程式模擬鍵盤按鍵時，電子書能接收到訊號。
...
### 步驟 4：開始下載
1. 程式會顯示 `✅ 成功連接到 Chrome！` 並自動鎖定您的電子書分頁。
2. 依照提示輸入該本書的 **總頁數** (例如：300)。
3. 程式開始自動截圖並翻頁。
...
| **只有截圖沒有翻頁** | 瀏覽器沒有「Focus (焦點)」 | 在執行 Python 程式前，務必用滑鼠**點一下電子書的頁面中央**。 |
